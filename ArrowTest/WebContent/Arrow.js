...
function objArrowLight(ctx, x1, y1, x2, y2, dir, sw) {
	this.ctx = ctx;
	this.x1 = x1;
	this.y1 = y1;
	this.x2 = x2;
	...
	} else if (this.dir == DIR_REV) {
		this.angle = f_getAngle(x1, y1, x2, y2) + 180;
	}
	this.lightCur = 0;
	this.arrowCur = 0;
	this.lightALL = "OFF";
...
objArrowLight.prototype.setDir = function(dir) {
	this.dir = dir;
	if (this.dir == DIR_FWD) {
		this.angle = f_getAngle(this.x1, this.y1, this.x2, this.y2);
	} else if (this.dir == DIR_REV) {
		this.angle = f_getAngle(this.x1, this.y1, this.x2, this.y2) + 180;
	}
	...
	if(this.coWait==0){
		...
		case DIR_FWD:
			for (i = 0; i < this.arrCo.length; i++) {
				drawArrowOne(this.arrCo[i][0], this.arrCo[i][1], this.angle, this.sw, this.arrCo[i][2]);
			}
			break;
		case DIR_REV:
			var reverse = [...this.arrCo].reverse();
			for (i = 0; i < reverse.length; i++) {
				drawArrowOne(reverse[i][0], reverse[i][1], this.angle, this.sw, this.arrCo[i][2]);
			}
			break;
		}
	}else{
